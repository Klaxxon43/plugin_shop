...
def show_tg_settings(message: types.Message):
    ...
    kb.add(
        ...
        InlineKeyboardButton("🔑 Токен LOLZ TEAM", callback_data="tg_lolz_token"),
        InlineKeyboardButton("📋 Заказы", callback_data="tg_orders"),
        InlineKeyboardButton("🔍 Настройка происхождений", callback_data="tg_origin"),
        InlineKeyboardButton("💬 Шаблоны ответов", callback_data="tg_message_templates"),
        InlineKeyboardButton("💻 Настройка покупки", callback_data="tg_buy"),
...
def init_commands(c_: Cardinal):
    ...
    def handle_all_callbacks(call: types.CallbackQuery):
        ...
        if call.data.startswith("tg_edit_country_name_"):
            ...
            admin_menu(call)
        elif call.data == "tg_buy":
            buy_menu(call)
        elif call.data == "tg_buy_on":
            buy_menu_on(call)
        elif call.data == "tg_buy_off":
            buy_menu_off(call)
    ...
    @bot.callback_query_handler(func=lambda call: call.data == "tg_buy")
    def buy_menu(call: types.CallbackQuery):
        kb = InlineKeyboardMarkup(row_width=1)
        current_status = "✅ Включены" if config["auto_returns"] else "❌ Выключены"

        kb.add(
            InlineKeyboardButton("✅ Покупать самый дешевый", callback_data="tg_buy_on"),
            InlineKeyboardButton("✅ Проверять аккаунты", callback_data="tg_buy_off")
        )
        kb.add(InlineKeyboardButton("🔙 Назад", callback_data="tg_back_to_main"))

        bot.edit_message_text(
            f"💻 Настройки покупки \n\n"
            "1. Покупать самый дешевый: При включении всегда выбирает самый дешевый подходящий аккаунт.\n2. Проверять аккаунты: Проверяет аккаунты перед покупкой на блокировки и проблемы. При включении покупает аккаунты без спамблоков\n\nТекущие настройки:",
            call.message.chat.id,
            call.message.message_id,
            reply_markup=kb
        ) 
            ...
            try:
                ...
                if not purchase_success:
                    if insufficient_funds:
                        ...
                        logger.error(f"{LOGGER_PREFIX} Не удалось купить ни один аккаунт")
                        message_text = f"Спасибо за покупку! Вы приобрели телеграм аккаунт с ID: {tg_id}.{country_info}\n\nК сожалению, все аккаунты данной страны были проданы. Наш администратор свяжется с вами в ближайшее время и сделает возврат. Спасибо за ожидание"
